...
export class MyActionDefaults implements ActionDefaults<CombatActions> {
  ...
  kill(target?: Monster | Location) {
    ...
    if (
      target && target instanceof Monster &&
    ...
    if (target && target instanceof Monster && target.physicalResistance >= 70)
      return this.delevel().skill($skill`Saucegeyser`).repeat();
  ...
  killHard(target?: Monster | Location) {
    if (
      ...
      return this.delevel().skill($skill`Lunging Thrust-Smack`).repeat();
    }
  }

  ignoreNoBanish() { return this.ignore(); }
  killFree() { return this.abort(); } // Abort if no resource provided
  banish() { return this.abort(); } // Abort if no resource provided
  abort() { return new Macro().abort(); }
